...
void remove_treasure(const char *hunt_path, int id) {
    ...
    while ((comoara = readdir(dir)) != NULL) {
	  ...
	  strcat(comoara_path,comoara->d_name);
            ...
            COMOARA c;
            while (read(fd, &c, sizeof(COMOARA)) == sizeof(COMOARA)) {
	      if (c.id != id) { //retinem doar comoriile pe care le pastram
                    treasure[count++] = c;
                }
            }
            close(fd);
	      ...
	      write(fd, &treasure[i], sizeof(COMOARA)); //rescriem fisierul de comoara doar cu comorile ramase (in caz ca sunt mai multe comori intr-un fisier)
            }
            close(fd);
        }
    }
    ...
    int log_fp=open(log_path, O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if(log_fp==-1)
      {
	char *eroare="eroare in remove_treasure.\n";
	write(1,eroare,strlen(eroare));
	exit(1);
      }
    
    //aici
    int aux_log_fd = open("aux_log.txt", O_WRONLY | O_CREAT | O_TRUNC, 0644);//creem un fisier temporal al logg-ului pentru a scrie in el ce ramane
    if (aux_log_fd == -1) {
        close(log_fp);
        exit(1);
    }
    ...
    char buf[20];
    int cnt=0;
    int x=id; //facem id-ul din int in char
    while(x){
      buf[cnt++]=(x%10)+'0';
      x=x/10;
    }
    buf[cnt]='\0';
    strcpy(inceput,"ID: ");
    strcat(inceput,buf); //salvam inceputul liniilor pe care vrem sa le stergem
    
    while ((size= read(log_fp, buffer, sizeof(buffer))) > 0) {//cat timp avem ce citi din fisierul logg
        for (int i = 0; i < size; ++i) {
            linie[index++] = buffer[i];
            if (buffer[i] == '\n') {
	      linie[index] = '\0';//salvam liniile din logg
	      if (strstr(linie, inceput) == NULL) {//daca linia nu are inceput id-ul nostru atunci o scriem in fisierul temporal
                    write(aux_log_fd, linie, strlen(linie));
                    write(aux_log_fd, "\n", 1);
                }
                index = 0;
            }
